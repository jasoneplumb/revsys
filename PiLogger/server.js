...
var log = 'PiLogger';
var ledState = 0;
store.create(log, 'ledState', ledState);
...
app.get('/inputs/:id', function (req, res) {
  ...
  for (i in inputs) {
    if ((req.params.id === inputs[i].input)) {
      ...
      // Finally, send a request object as a JSON string to client
      res.send(requests[i]);

      // Record the new state of the LED in the log
      store.capture(log, ledState.toString());

      return;
    }
  }

  console.log('invalid input port (' + req.params.id + ')');
